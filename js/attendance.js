...
function loadSessions() {
    ...
    $.ajax({
        ...
        error: function(xhr, status, err) {
            var msg = xhr && xhr.responseText ? xhr.responseText : (status + ' ' + err);
            alert("OOPS! " + (msg.length > 200 ? msg.substr(0, 200) + "..." : msg));
            console.error('getSession error', status, err, xhr);
        }
    });
}
...
function fetchFacultyCourses(facid, sessionid) {
    ...
    $.ajax({
        ...
        error: function(xhr, status, err) {
            var msg = xhr && xhr.responseText ? xhr.responseText : (status + ' ' + err);
            alert("OOPS! getFacultyCourses: " + (msg.length > 200 ? msg.substr(0, 200) + "..." : msg));
            console.error('getFacultyCourses error', status, err, xhr);
        }
    });
}
...
function fetchStudentList(sessionid, classid, facid, ondate) {
    ...
    $.ajax({
        ...
        error: function(xhr, status, err) {
            var msg = xhr && xhr.responseText ? xhr.responseText : (status + ' ' + err);
            alert("OOPS! getStudentList: " + (msg.length > 200 ? msg.substr(0, 200) + "..." : msg));
            console.error('getStudentList error', status, err, xhr);
        }
    });
}
...
function saveAttendance(studentid, courseid, facultyid, sessionid, ondate, ispresent) {
    ...
    $.ajax({
        ...
        error: function(xhr, status, err) {
            var msg = xhr && xhr.responseText ? xhr.responseText : (status + ' ' + err);
            alert("OOPS! saveAttendance: " + (msg.length > 200 ? msg.substr(0, 200) + "..." : msg));
            console.error('saveAttendance error', status, err, xhr);
        }
    });
}
...
function downloadCSV(sessionid, classid, facid) {
    ...
    $.ajax({
       ...
       `;
        ...
        error: function(xhr, status, err) {
            var msg = xhr && xhr.responseText ? xhr.responseText : (status + ' ' + err);
            alert("OOPS! downloadReport: " + (msg.length > 200 ? msg.substr(0, 200) + "..." : msg));
            console.error('downloadReport error', status, err, xhr);
        }
    });
}
...
$(function(e) {
    ...
    $(document).on("change", "#ddlclass", function(e) {
        ...
        if (si != -1) {
            ...
            console.log("Fetching courses - Faculty ID:", facid, "Session ID:", sessionid);
