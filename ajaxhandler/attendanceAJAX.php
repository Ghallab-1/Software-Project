...
if(isset($_REQUEST['action']))
{
    ...
    if($action=="getFacultyCourses")
    {
        //fetch the courses taken by fac in sess
         $facid=isset($_POST['facid']) ? $_POST['facid'] : '';
         $sessionid=isset($_POST['sessionid']) ? $_POST['sessionid'] : '';
         error_log("getFacultyCourses called - facid: $facid, sessionid: $sessionid");
         $dbo=new Database();
         $fo=new faculty_details();
         $rv=$fo->getCoursesInASession($dbo,$sessionid,$facid);
         error_log("getFacultyCourses result count: " . count($rv));
         //$rv=[];
